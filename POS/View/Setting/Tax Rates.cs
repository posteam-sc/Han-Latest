    ...
    public partial class Taxes : Form
    {
        ...
        private void Taxes_Load(object sender, EventArgs e)
        {
            ...
            if (notbackoffice)
            {
                ...
                dgvTaxList.Columns[4].Visible = false;
            }
            else
            {
                dgvTaxList.Columns[4].Visible = true;
            }
            dgvTaxList.AutoGenerateColumns = false;
            dgvTaxList.DataSource = posEntity.Taxes.Where(x=>x.IsDelete==false).ToList();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (!hasError)
            {
                ...
                {
                    Tax taxObj = new Tax();
                    Tax taxObj2 = (from t in posEntity.Taxes where t.Name == txtName.Text && t.IsDelete==false select t).FirstOrDefault();
                    if (taxObj2 == null)
                    {

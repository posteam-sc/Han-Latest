    ...
    public partial class newMemberType : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (txtType.Text.Trim() != string.Empty)
            {
                ...
                APP_Data.MemberType bObj = (from b in posEntity.MemberTypes where b.Name == txtType.Text where b.IsDelete==false select b).FirstOrDefault();
                if (bObj == null)
                {
                    ...
                    if (!isEdit)
                    {
                        if (controller.MemberRule.Add || MemberShip.isAdmin)
                        {
                            ...
                            dgvMemberList.DataSource = (from m in posEntity.MemberTypes where m.IsDelete==false orderby m.Id descending select m).ToList();
                    ...
                    if (System.Windows.Forms.Application.OpenForms["MemberRule"] != null)
                    {
                        ...
                        newForm.LoadPromotionMemberCbo(memType);
                        this.Close();
                    }
                    ...
                    if (controller.MemberRule.EditOrDelete || MemberShip.isAdmin)
                    {
                        ...
                        if (result.Equals(DialogResult.OK))
                        {
                            ...
                            if (count < 1)
                            {
                                ...
                                posEntity.SaveChanges();
                                dgvMemberList.DataSource = (from m in posEntity.MemberTypes where m.IsDelete == false select m).ToList();

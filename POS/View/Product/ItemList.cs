...
namespace POS
{
    public partial class ItemList : Form
    {
        ...
        List<APP_Data.ProductCategory> ProductCategoryList = new List<APP_Data.ProductCategory>();
        List<APP_Data.ProductSubCategory> ProductSubCategoryList = new List<APP_Data.ProductSubCategory>();
        ...
        {
            AllProductList = entity.Products.ToList();
            ProductCategoryList = entity.ProductCategories.ToList();
            ProductSubCategoryList = entity.ProductSubCategories.ToList();
            ...
            if (notbackoffice)
            {
               ...
               // btnAdd.Visible = false;
                ...
                dgvItemList.Location=new Point(10,140);
                dgvItemList.Size = new System.Drawing.Size(1315, 530);
                dgvItemList.Columns[10].Visible = false;
            }
            else
            {
               // btnAdd.Enabled = true;
           ...
           // Utility.BindBrand(cboBrand);
            Utility.BindBrand(cboBrand, BrandList);
            ...
            pMainCatList.AddRange(ProductCategoryList.Where(MainCategory=> MainCategory.IsDelete == false).ToList());
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (controller.Product.Add || MemberShip.isAdmin)
            {
                ...
                newForm.AllProductList = AllProductList;
                newForm.ProductCategoryList = ProductCategoryList;
                newForm.ProductSubCategoryList = ProductSubCategoryList;
                newForm.BrandList = BrandList;
                newForm.Show();
            }
            else
            {
        ...
        private void LoadData()
        {
            ...
            productList = AllProductList.Where(p => ((rdAll.Checked == true && 1 == 1) || (rdNonConsignment.Checked == true && p.IsConsignment == false) || (rdConsignment.Checked == true && p.IsConsignment == true))
        ...
        private void dgvItemList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == 8)
                {
                    ProductBatchDetial batchdetailForm = new ProductBatchDetial();
                    batchdetailForm.ProductId = currentProductId;
                    batchdetailForm.ProdName = dgvItemList.Rows[e.RowIndex].Cells[4].Value.ToString();
                ...
                else if (e.ColumnIndex == 11)
                {
                    if (controller.Product.EditOrDelete || MemberShip.isAdmin == true)
                    {
                        ...
                        if (result.Equals(DialogResult.OK))
                        {
                            ...
                            productObj = AllProductList.Where(p=> p.Id == productObj.Id).FirstOrDefault();
        ...
        private void cboMainCategory_SelectedValueChanged(object sender, EventArgs e)
        {
            if (cboMainCategory.SelectedIndex != 0 && cboMainCategory.SelectedIndex != 1)
            {
                ...
                SubCategoryObj1.Name = "Select";
                pSubCatList.Add(SubCategoryObj1);
                pSubCatList.AddRange(ProductSubCategoryList.Where(c=> c.ProductCategoryId == productCategoryId).ToList());
        ...
        public void DataBind()
        {
            ...
            //dgvItemList.DataSource = entity.Products.Where(x=>x.IsDeleted==false).ToList();
            dgvItemList.DataSource = AllProductList.Where(x=>x.IsDeleted == false).ToList();
        }
        ...
        private void cboMainCategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboMainCategory.SelectedIndex != 0)
            {
                ...
                //pSubCatList.AddRange((from c in entity.ProductSubCategories where c.ProductCategoryId == productCategoryId where c.IsDelete==false select c).ToList());
                pSubCatList.AddRange(ProductSubCategoryList.Where(c => c.ProductCategoryId == productCategoryId && c.IsDelete == false).ToList());
                cboSubCategory.DataSource = pSubCatList;
                cboSubCategory.DisplayMember = "Name";
                cboSubCategory.ValueMember = "Id";
                cboSubCategory.AutoCompleteMode = AutoCompleteMode.SuggestAppend;

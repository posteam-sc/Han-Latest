...
namespace POS
{
    public partial class ProductSubCategory : Form
    {
        ...
        private void ProductType_Load(object sender, EventArgs e)
        {
            ...
            {
                dgvProductList.Columns[5].Visible = true;
            }
            ...
            pMCategoriesList.AddRange((from mCat in posEntity.ProductCategories where mCat.IsDelete==false select mCat).ToList());
            ...
            dgvProductList.DataSource = (from pType in posEntity.ProductSubCategories where pType.IsDelete==false orderby pType.Id descending select pType).ToList();
        }
            ...
            if (MainCategoryId > 0)
            {
                ...
                if (hasError==false)
                {
                            ...
                            if (!isEdit)
                            {
                                ...
                                {
                                    APP_Data.ProductSubCategory pSubCat = (from psCat in posEntity.ProductSubCategories where psCat.Name == txtName.Text && psCat.ProductCategoryId == MainCategoryId && psCat.IsDelete==false select psCat).FirstOrDefault();
                    ...
                    if (controller.SubCategory.EditOrDelete || MemberShip.isAdmin)
                    {
                        ...
                        if (result.Equals(DialogResult.OK))
                        {
                            ...
                            if (count < 1)
                            {
                                ...
                                posEntity.SaveChanges();
                                dgvProductList.DataSource = (from pt in posEntity.ProductSubCategories where pt.IsDelete == false select pt).ToList();

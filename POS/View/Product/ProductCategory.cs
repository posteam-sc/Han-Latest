    ...
    public partial class ProductCategory : Form
    {
        ...
        private void ProductCategory_Load(object sender, EventArgs e)
        {
            ...
            if (notbackoffice)
            {
                ...
                dgvProductCList.Columns[3].Visible = false;
            }
            else
            {
                dgvProductCList.Columns[3].Visible = true;
            }
            dgvProductCList.AutoGenerateColumns = false;
            dgvProductCList.DataSource = (from pType in posEntity.ProductCategories where pType.IsDelete==false orderby pType.Id descending select pType).ToList();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (controller.Category.Add || MemberShip.isAdmin)
            {
                ...
                if (hasError!=true)
                {
                    ...
                    APP_Data.ProductCategory pCObj = (from pC in posEntity.ProductCategories where pC.Name == txtName.Text && pC.Name!=OldName && pC.IsDelete==false select pC).FirstOrDefault();
                    if (pCObj == null)
                    {
                        //New Product
                            if (!isEdit)
                        ...
                        if (currentId != 1045)
                        {
                            ...
                            if (result.Equals(DialogResult.OK))
                            {
                                ...
                                if (count < 1)
                                {
                                    ...
                                    posEntity.SaveChanges();
                                    dgvProductCList.DataSource = (from pt in posEntity.ProductCategories where pt.IsDelete == false select pt).ToList();
                                    #region active new Product
                                    if (System.Windows.Forms.Application.OpenForms["NewProduct"] != null)
                                    {
                                        NewProduct newForm = (NewProduct)System.Windows.Forms.Application.OpenForms["NewProduct"];
